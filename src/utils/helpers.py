...
if __name__ == "__main__":
  ...
  assert (bytes_or_hexstr_to_decimal((1234).to_bytes(16, "big"),
                                     "bytes") == 1234)
  assert (bytes_or_hexstr_to_decimal((1234).to_bytes(16, "big").hex(),
                                     "str") == 1234)

  print("Test bytes_or_hexstr_to_decimal passed\n")

  assert (ephid_bytes_or_hexstr_to_decimal(
      (567856785678567856785678).to_bytes(16,
                                          "big")) == 567856785678567856785678)
  assert (ephid_bytes_or_hexstr_to_decimal((567856785678567856785678).to_bytes(
      16, "big").hex()) == 567856785678567856785678)

  print("Test ephid_bytes_or_hexstr_to_decimal passed\n")

  test_bytes = b"\x78\x3f\x93\xfa\x14\x76\x73\x7d\xc9\x0e"
  assert (ephid_decimal_to_bytes(567856785678567856785678) == bytes(
      b"\x00" * (16 - len(test_bytes))) + test_bytes)
  assert (ephid_decimal_to_bytes(567856785678567856785678).hex() == "00" *
          (16 - len(test_bytes)) + hex(567856785678567856785678)[2:])
