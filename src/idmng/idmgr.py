...
class IDManager(UDPManager):
    ...
    def share_EphID(self, ephid, interval, ip, port, parts = 6, threadshold = 3):
        ...
        secret, shares = make_shares(ephid_bytes_or_hexstr_to_decimal(ephid), threadshold, parts)

        msg = Message()
        hash_tag_len = msg._tag_len

        # hex string type
        hash_tag = sha256(ephid \
            if isinstance(ephid, bytes) \
            else ephid.encode("utf-8")).hexdigest()[:hash_tag_len * 2]
    ...
    def wait_for_secret(self, ip, port, threshold, bufsz = 1024, filter_self = True):
        ...
        self.logger.info(f"Got the part {int(sec_id.hex(), 16)}, secret: {secret.hex()} from: {hash_tag.hex()}")
        ...
        if (len(secret_shares) >= threshold):
            ...
            new_hash_tag = sha256(ephid_of_other \
                if isinstance(ephid_of_other, bytes) \
                else ephid_of_other.encode("utf-8")).hexdigest()[:len(hash_tag) * 2]
            ...
            self.logger.info(f"Received {len(secret_shares)} parts not enoungh to reconst the EphID")
            encntid = None
